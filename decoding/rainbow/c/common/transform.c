...
extern "C"
{
  ...
  int BrotliTransformDictionaryWord(uint8_t *dst, const uint8_t *word, int len,
                                    const BrotliTransforms *transforms, int transform_idx, unsigned long long *index, meta *buf, int offset)
  {
    int idx = 0;
    ...
    buf[*index - 1].index = offset;
    {
      const int t = type;
      int i = 0;
      if (t <= BROTLI_TRANSFORM_OMIT_LAST_9) // word[.....] => string word[...] [...]
      {
        len -= t;
        buf[*index - 2].copy_len = len;
      }
      ...
      { // word[.....] => [...] word[....]
        ...
        len -= skip;
        buf[*index - 2].copy_len = len;
        buf[*index - 1].index += skip; //更新索引
      }
      while (i < len)
      {
        dst[idx++] = word[i++];
      }
      if (t == BROTLI_TRANSFORM_UPPERCASE_FIRST) //首字母大写
      {
        buf[*index - 3].count += 1;
        buf[*index - 2].copy_len -= 1;
        buf[*index - 1].index += 1;
        char c = dst[idx - len];
        ToUpperCase(&dst[idx - len]);
      }
      else if (t == BROTLI_TRANSFORM_UPPERCASE_ALL) //全大写
      {
        buf[*index - 3].count += len;
        buf[*index - 2].copy_len -= len;
        buf[*index - 1].index = 122784;
      ...
      {
        printf("BROTLI_TRANSFORM_SHIFT_FIRST\n");
      ...
      {
        printf("BROTLI_TRANSFORM_SHIFT_ALL\n");
        uint16_t param = (uint16_t)(transforms->params[transform_idx * 2] + (transforms->params[transform_idx * 2 + 1] << 8u));
    ...
    //*index += 1;
    {
      ...
      if (suffix_len > 0)
      { //额外记录后缀
        buf[*index].count = suffix_len;
        *index += 1;
        buf[*index].copy_len = 0;
        *index += 1;
        buf[*index].index = 122784;
        *index += 1;
      }

      while (suffix_len--) //添加后缀
      {
        dst[idx++] = *suffix;
        suffix++;
      }
