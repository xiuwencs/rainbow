...
#include "./prefix.h"
#include "./state.h"
extern unsigned long long global_index;
extern meta *buf;
...
#define BROTLI_SAFE(METHOD)                       \
  ...
  CommandBegin:
    ...
    /*BROTLI_LOG1(("[ProcessCommandsInternal] pos = %d insert = %d copy = %d\n",
                pos, i, s->copy_length));*/
    /*BROTLI_LOG1(("%d %d %d\n",
                pos, i, s->copy_length));*/
    buf[global_index].count = i;
    global_index++;
    buf[global_index].copy_len = s->copy_length;
    global_index++;
    buf[global_index].index = 122784;
    global_index++;
    if (i == 0)
    {
      goto CommandPostDecodeLiterals;
  ...
  CommandPostDecodeLiterals:
        ...
        if (transform_idx < (int)transforms->num_transforms)
        {
          ...
          if (transform_idx == transforms->cutOffTransforms[0])
          {
            ...
            BROTLI_LOG(("[ProcessCommandsInternal] dictionary word: [%.*s]\n",
                        len, word));
            buf[global_index - 1].index = offset;
          }
          else
          {
            len = BrotliTransformDictionaryWord(&s->ringbuffer[pos], word, len,
                                                transforms, transform_idx, &global_index, buf, offset);
            BROTLI_LOG(("[ProcessCommandsInternal] dictionary word: [%.*s],"
    ...
    {
      buf[global_index - 1].index = -s->distance_code; //取负数表示动态字典
