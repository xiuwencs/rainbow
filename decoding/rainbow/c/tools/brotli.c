...
int g = 0;
unsigned long long global_index = 0L;
long spend = 0;
typedef struct {
  /* Parameters */
  int quality;
  ...
  uint8_t* input;
  uint8_t* output;
  const char* current_input_path; //输入路径
  const char* current_output_path; //输出路径
  ...
  while (NextFile(context)) {
	...
	//strcat(file_name,".out");
        ...
        /*for(int i = 0;i<global_index;i+=3){
          printf("%d %d\n",context->meta_buff[i],context->meta_buff[i+1]);
        }*/
        //fwrite(context->meta_buff,sizeof(meta),global_index,file);
        global_index = 0;
    } 
...
int main(int argc, char** argv) {
  ...
  context.meta_buff = (meta *)malloc(1024 * 1024 * 1000);

  buf = context.meta_buff;
